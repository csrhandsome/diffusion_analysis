...
from diffusion.model.diffusion.cnn_based import ConditionalUnet1D
...
def threedim_state_and_vision_train_model():
    ...
    torch.save(checkpoint, "data/pretrained/cnn_based_model.ckpt")
    return model,noise_scheduler,ema,optimizer
...
def threedim_state_and_vision_test_model():
    ...
    step_idx=0
    # 读取ckpt模型参数
    model_dict = torch.load("data/pretrained/cnn_based_model.ckpt", map_location='cuda',weights_only=True)
    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # 创建模型
    model,noise_scheduler,ema,optimizer=threedim_state_and_vision_create_model()
    ...
    with tqdm(total=200,desc='Output',leave=False) as pbar:
        while not done:
            ...
            #simlation_action=np.concatenate((simulation_action,action),axis=0)
