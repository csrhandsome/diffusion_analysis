...
from diffusion.model.diffusion.rdt_model import *
...
def threedim_state_create_model():
    ...
    model = RDT(
        ...
        hidden_size=hidden_size,              # 里面所有的最后一个维度都应为540
        depth=28,                     
        num_heads=9,                  # 改为9 (531/9 = 59)
        # ---------cond条件添加--------- transformer里面的cond需要用交叉注意力     
        img_cond_len=vision_feature_dim,      
        img_pos_embed_config={
            'vision': vision_feature_dim
        },
        max_lang_cond_len=6+6+6+2,
    ...
    '''noise = torch.randn((1, 8, 7))#观测两个现有的时间维度,预测8个时间维度
    ...
    """
        ...
        """
    # 仔细回忆一下输入，我的x应该为添加了噪声的action，所以形状是没有发生变化的。
    # --------开始罗列forward需要的输入--------
    B = 8
    noise = torch.randn((B,state_horizon+1,hidden_size))# 这里暂定+1
    state = noise.to(device)
    diffusion_iter = torch.zeros((B,)).to(device)
    freq = torch.full((B,), 60.0).to(device)
    lang_c = torch.zeros((B,6+6+6+2)).to(device)
    img_c = torch.zeros((B,512)).to(device)
    output=model(x=state, freq=freq, t=diffusion_iter, lang_c=lang_c, img_c=img_c)
    #print(f'output is noise is {output}')
    denoised=noise-output
    print(f'model created')
    return model,noise_scheduler,ema,optimizer

def threedim_state_train_model():
    print('-------------Training state model-------------')
    model,noise_scheduler,ema,optimizer=threedim_state_create_model()
        ...
        for epoch in tqdm_epoch:
            ...
            with tqdm(dataloader,desc='Batch', leave=False) as tqdm_batch:#leave=False 进度条不leave,一直在原地刷新
                for batch in tqdm_batch:#batch为batch_size的个sample_sequence
                    epoch_state=batch['state'].to(device).detach().float()#epoch_state is torch.Size([Batch_size, 2, 6])
                    epoch_action=batch['action'].to(device).detach().float()#epoch_action is torch.Size([Batch_size, 8, 7])  
                    ...
                    noisy_actions = noise_scheduler.add_noise(
                        epoch_action, noise, timesteps)# 返回添加噪音后的action 
                    # 噪声残差预测值
                    noise_pred = model(
    ...
    torch.save(checkpoint, "rdt_based_model.ckpt")
    noise_scheduler.save_pretrained("noise_scheduler_state")
    return model,noise_scheduler,ema,optimizer

def threedim_state_test_model():
    # 读取数据
    dataloader,mydataset=create_MagiClaw_dataloader()
    max_steps=200
    step_idx=0
    # 读取ckpt模型参数
    model_dict = torch.load("rdt_based_model.ckpt", map_location='cuda',weights_only=True)
    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # 创建模型
    model,noise_scheduler,ema,optimizer=threedim_state_create_model()
    ...
    state=np.array((-0.14658429,0.09669642,-0.2047276,-1.34936731,0.23016844,0.24176986))#这个为初始的state,也就是pose
    state_deque=collections.deque([state]*state_horizon,maxlen=state_horizon)
    done=False
    visualizer = ActionVisualizer3D()
    with tqdm(total=200,desc='Output',leave=False) as pbar:
        while not done:
            ...
            for i in range(len(action)):
                ...
                visualizer.update(current_action[:3])
                pbar.update(1)
                step_idx += 1
