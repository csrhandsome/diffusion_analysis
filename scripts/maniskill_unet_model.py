...
from data.magiclaw_state_vec import STATE_VEC_IDX_MAPPING
...
MANISKILL_INDICES = [
    STATE_VEC_IDX_MAPPING[f"ur5_joint_{i}_pos"] for i in range(6)# 获取其右臂的6个关节位置索引
] + [
    STATE_VEC_IDX_MAPPING[f"gripper_angle"]# 末端夹爪开合角度
]


def create_model(args, pretrained, **kwargs):# 多余的参数传给了kwargs，并且形成了一个字典
    model = RoboticDiffusionUnetModel(args, **kwargs)
...
class RoboticDiffusionUnetModel(object):
    """A wrapper for the RUT model, which handles
    ...
    def get_policy(self):
        ...
        img_cond_len = (self.args["common"]["img_history_size"] 
                        ...
                        * 1)# 将resnet最后一层替换成Identity了，因此self.vision_model.num_patches=1
        ...
        _model = UNETRunner(
            ...
            img_dim=self.args["model"]["img_token_dim"],
            state_dim=self.args["model"]["state_token_dim"],
    ...
    def get_vision_encoder(self, pretrained_vision_encoder_name_or_path):
        if pretrained_vision_encoder_name_or_path is 'resnet':
            vision_encoder = get_resnet('resnet18',weights="r3m")# 加载r3m权重的resnet18模型
