...
class Conv1dBlock(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, kernel_size, n_groups=8):
        ...
        self.block = nn.Sequential(
            ...
            nn.GroupNorm(n_groups, out_channels),#归一化
            nn.Mish(),#激活函数
        )
...
class ConditionalResidualBlock1D(nn.Module):
    '''条件残差块
    最基础的残差连接
    output = F(x) + x  # F(x)是经过神经网络变换后的结果,x是输入

    Input(x) -----> Conv1D -----> FiLM -----> Conv1D ---(+)---> Output
     |                                              |
     |                                              |(残差连接)
     +-------------> residual_conv ------------------>
    允许网络根据条件信息(cond)调整特征
    通过scale和bias实现特征的动态调整
    增强网络的表达能力
    '''
    def __init__(self,
            in_channels,
    ...
    def forward(self, x, cond):
        ...
        # 步骤3: FiLM调制 简单的二次函数的调制
        out = scale * out + bias  # 特征调制
...
class ConditionalUnet1D(nn.Module):
    def __init__(self,
        ...
        global_cond_dim: Dim of global conditioning applied with FiLM全局条件维度,应用于FILM
          in addition to diffusion step embedding. This is usually obs_horizon * obs_dim
        diffusion_step_embed_dim: Size of positional encoding for diffusion iteration k  扩散迭代k的位置编码大小
        down_dims: Channel size for each UNet level. 每个UNet级别的通道大小
          The length of this array determines numebr of levels.
        kernel_size: Conv kernel size 卷积核大小
    ...
    def forward(self,
            sample: torch.Tensor,
            timestep: Union[torch.Tensor, float, int],# 接受任意一个
        ...
        x = sample
        h = []
        for idx, (resnet, resnet2, downsample) in enumerate(self.down_modules):# 逐一的过不同的神经网络层
