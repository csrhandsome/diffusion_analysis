...
class MagiclawVLADataset:
    ...
    def __init__(self) -> None:
        ...
        self.STATE_DIM = config['common']['state_dim']# 128 
        # 计算episodes
        self.episode_ends=[]
        start_idx = 0
        episode_lens = load_episodes_length(self.data_path)
        for i in range(len(episode_lens)):
            start_idx += episode_lens[i]
            self.episode_ends.append(start_idx)
    ...
    def parse_file(self, file_path):
        ...
        def parse_img(key):
            ...
            for i in range(max(step_id-self.IMG_HISORY_SIZE+1, 0), step_id+1):
                img = self.data['observations']['images'][key][i]
    ...
    def parse_file_state_only(self, file_path):
        ...
        target_qpos = data['action'][:] / np.array(
            [[1, 1, 1, 1, 1, 1, 11.8997, 1, 1, 1, 1, 1, 1, 13.9231]] 
        )
